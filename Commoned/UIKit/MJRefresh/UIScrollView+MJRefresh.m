...
- (void)addHeaderWithTarget:(id)target action:(SEL)action
{
    ...
    if(self.header.beginRefreshingAction==action&&self.header.beginRefreshingTaget==target)return;
    // 2.设置目标和回调方法
...
- (void)addFooterWithTarget:(id)target action:(SEL)action
{
    ...
    if(self.footer.beginRefreshingTaget == target && self.footer.beginRefreshingAction == action)return;
    // 2.设置目标和回调方法
