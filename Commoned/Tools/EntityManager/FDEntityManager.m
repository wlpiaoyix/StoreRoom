...
-(id) init{
    if(self = [super init]){
        self->db = [FMDatabase databaseWithPath:[NSString stringWithFormat:@"%@/%@",[Common getDocumentDir],@"entities.db"]];
        if(![db open]){
...
-(id) initWithDBName:(NSString *) dbName{
    if(self = [super init]){
        self->db = [FMDatabase databaseWithPath:[NSString stringWithFormat:@"%@/%@",[Common getDocumentDir],dbName]];
        if(![db open]){
            printf("open database faild!\n");
            return nil;
        }
