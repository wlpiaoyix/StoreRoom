...
-(BOOL) resumeDownLoad{
    ...
    [downloadTask resume];
    currenttime = [Common getTimeInterval];
    return true;
}
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location{
    ...
    @finally {
        BOOL isDirectory;
        if ([Common fileExistsAtPath:relativePath isDirectory:&isDirectory isCreated:false]&&!isDirectory) {
            NSError *err;
...
-(NSURLSession*) createSession{
    ...
    if ([Common getSystemVersion]>=8.0) {
