...
- (void)layoutSubviews {
    ...
    if ([NSString isEnabled:label.text]) {
        labelSize = [Common getBoundingRectWithSize:label.text font:label.font size:CGSizeMake(999, 20)];
        if (labelSize.width<bounds.size.width/2) {
            labelSize = [Common getBoundingRectWithSize:label.text font:label.font size:CGSizeMake(bounds.size.width, 999)];
        }
    }
